...
def classify(path_to_palmline_image):
    ...
    skeleton = skeletonize(palmline_img > 0).astype(np.uint8) * 255
    if len(skeleton.shape) == 2:  # Single-channel image
        skel_img = cv2.cvtColor(skeleton, cv2.COLOR_GRAY2BGR)  # Convert to BGR
    else:
        print("The skeleton image is already in BGR format.")
        skel_img = skeleton 
